...
Log.Logger = new LoggerConfiguration()
                    .MinimumLevel.Debug()
                    .WriteTo.Console(restrictedToMinimumLevel: Serilog.Events.LogEventLevel.Debug, outputTemplate: "[{Timestamp:HH:mm:ss} {Level:u3}] {Message:lj} <s:{SourceContext}>{NewLine}{Exception}")
                    .WriteTo.MSSqlServer(connectionString: builder.Configuration.GetConnectionString("MemeMakerConnection"), tableName:"Logs", autoCreateSqlTable: true, restrictedToMinimumLevel:Serilog.Events.LogEventLevel.Warning)
                    .WriteTo.File(restrictedToMinimumLevel: Serilog.Events.LogEventLevel.Information, flushToDiskInterval: new TimeSpan(0,0,10), path:builder.Configuration["DiskPaths:LogFilePath"], rollOnFileSizeLimit:true, fileSizeLimitBytes: 50100100, rollingInterval: RollingInterval.Day, retainedFileCountLimit: 5, buffered: false, outputTemplate: "{Timestamp:dd-MMM-yyyy HH:mm:ss.fff zzz} {Level:us} tid={ThreadId} {Message:lj}{NewLine}")
                    .CreateLogger();
